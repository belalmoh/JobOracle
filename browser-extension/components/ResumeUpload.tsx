...
import { FileText, UploadSimple, X, Spinner } from '@phosphor-icons/react';
    ...
    <Card className="border-border/50">
      ...
      <CardContent className="px-4 pb-4">
        {uploadState === 'idle' && (
          ...
          >
            <UploadSimple size={24} weight="duotone" className="text-muted-foreground" />
            <p className="text-xs text-muted-foreground text-center">
              Drop your resume here or <span className="text-primary font-medium">browse</span>
            </p>
            <p className="text-[10px] text-muted-foreground">PDF, DOCX, or TXT (max {MAX_SIZE_MB}MB)</p>
            <input
              ref={inputRef}
              type="file"
        ...
        {uploadState !== 'idle' && file && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 rounded-lg bg-muted/50 p-3">
              <FileText size={18} weight="fill" className="text-primary shrink-0" />
              <span className="text-xs font-medium truncate flex-1">{file.name}</span>
              {uploadState === 'uploading' && (
                <Spinner size={16} className="animate-spin text-muted-foreground shrink-0" />
              )}
